...
<section class="content-header">
    ...
    <div class="content px-3">
        ...
        <div class="card">
            <div class="card-body">
                <h2 >
                       {{-- اسم البنك {{ $bank_account->get_payment_type->name }}    --}}
                        اجمالي الرصيد <span class="badge badge-info" style="font-size: 20px">{{ $result_total }}</span> 
                </h2>
            </div>
        </div>
            ...
            <div class="card-body">
                <div id="tab1" style="max-height: calc(80vh - 150px); overflow-y: auto;">
                    ...
                    @foreach ($bank_account as $bank_accounts)
                            <tr>
                            @if(empty($bank_accounts->treasury_journal))
                                <td>---</td>
                            @else
                                <td>{{ $bank_accounts->treasury_journal }}</td>
                            @endif
                                <td>{{ number_format(  $bank_accounts->credit ,2) }}</td>
                                <td>{{ number_format( $bank_accounts->debit ,2)}}</td>
                                <td>{{ $bank_accounts->details }}</td>
                                <td>{{ $bank_accounts->created_at }}</td>
                            </tr>
