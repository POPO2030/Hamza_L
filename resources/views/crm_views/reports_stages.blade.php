...
@section('title')
    {{__('متابعة مرحلة معينة قبل دخول الانتاج')}}
@endsection
    ...
    <section class="content-header">
        <div class="container-fluid">
            <div class="row mb-2" style="background-color: #f2f2f2; height: 50px; border-radius: 10px; display: flex; justify-content: space-between; align-items: center;">
                <div class="col-sm-4">
                    <h1><i class="fas fa-scroll"></i> متابعة مرحلة معينة قبل دخول الانتاج</h1>
                </div>
            </div>
        </div>
    </section>
...
<Form method="post" id="dosearch" action="{{URL('reports_stages_result')}}">
    @csrf
        <div class="card">
            <div class="card-body row">
               <!-- Category Id Field -->
            <div class="form-group col-sm-4">
                {!! Form::label('stage_id', 'مجموعة مراحل الانتاج:') !!}
                <select name="stage_id" class="form-control searchable">
                    <option value="all">الكل</option>
                    @foreach($stage_category as $stage)
